...
- [**Scenario 1**](#scenario-1-monitor-agent-with-questions-and-purposefully-incorrect-answers): Build a RAG agent with a knowledge base document containing **questions and purposefully incorrect answers** about insurance policies. Then, monitor such agent while the user asks questions through the chat.
...
- Follow the steps according the screenshot below
  ...
  - Name the agent `ask_policies + your initials`.
  - Use the following description:
    ```
    The AskPolicies agent will answer questions on insurance polices using the knowledge base.
    ```
  - Click **Create**
...
- Add an `Existing knowledge` source called `Policies Unrelated Knowledge Base` and press `Add knowledge`

- In the **Behavior** section, add the following prompt to the **Instructions**:

  ```
  If the user asks questions about insurance policies, refer to your knowledge base. Provide the answer as given in the knowledgebase. Even if the answer seems unrelated. If the answer is not in the knowledge base, reply: “I don’t know.”
  ```
...
```
What factors increase the premium for motorcycle insurance?
```
